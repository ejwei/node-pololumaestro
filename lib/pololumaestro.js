 ...
 */
PololuMaestro.prototype.setTarget = function(device, channel, us, onComplete) {
	/*
	if(us < 640 || us > 2304) {
	...
	*/
	...
	LOG.debug("PololuMaestro","Device Number", device, "with Setting channel", channel, "target to", us, "microseconds");
	this._sendCommandDevices(device, 0x04, channel, us * 4, onComplete);
};
 ...
 */
PololuMaestro.prototype.setSpeed = function(device, channel, us, onComplete) {
	/*
	if(us < 0 || us > 255) {
	...
	*/
	...
	this._sendCommandDevices(device, 0x07, channel, us, onComplete);
};
...
PololuMaestro.prototype._sendCommandDevices = function(command, device, channel, value, onComplete) {
	this._serialPort.write([ 0xAA, device, command, channel ].concat(ByteUtils.toLowAndHighBits(value)), onComplete);
};
